...
impl DiscoveredModel {
    pub(crate) fn from_proto(
        ...
        let data_parallel_range = if let Some(parallelism) = server.parallelism.as_ref() {
            // TODO(vllm-proto-0.3): replace 0 with parallelism.data_parallel_size_local
            // once the released crate includes vllm-project/vllm#57116. Until then,
            // preserve complete-group discovery; hybrid ownership remains unavailable.
