...
<ParamField path="dynamo_frontend_failures_total" type="counter">
  Terminal request failures classified by the stable `class` and `reason` labels. Diagnostics and public messages are not metric labels. `DYN_METRICS_PREFIX` replaces the `dynamo_frontend` prefix when configured.
</ParamField>
