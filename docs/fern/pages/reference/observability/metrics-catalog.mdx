...
<ParamField path="dynamo_frontend_failures_total" type="counter">
  Terminal request failures classified by the stable `class` and `reason` labels. Diagnostics and public messages are not metric labels. The configured frontend metrics prefix, including `DYN_METRICS_PREFIX` or `--metrics-prefix`, replaces `dynamo_frontend`.
</ParamField>
